...
//Loop
    while(1){
        ...
        if (pushbtn == 0){                                      //Si está presionado y ya se completó un ciclo
            if (bandera == 1){
                bandera = 0;                                    //Volver a empezar
            }
        }
        if (pushbtn == 0 && bandera == 0){                              //Si se presionó y se puede empezar
...
void delay(uint32_t ms){
    for (i = 0; i < ms; i++){       //Ejecutar número de veces del argumento
